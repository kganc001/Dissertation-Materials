...
setwd(wd)
rm(wd)
 ad.dta("supreme court mood replication.dta")
...
#STATE VARIABLES
x0 <- ts$rep.policy_trans[1] #prey - policy choice
y0 <- ts$rep.mood[1] #predator - public opinion/mood
...
for(i in seq(1,d,1)){
  ...
  try(write.csv(fit2, paste("output",i,".csv", sep = "")))
